...
function MetricCell({
  ...
  const tones = {
    ...
    expense: { icon: "text-rose-500", value: "text-rose-500" },
    credit: { icon: "text-sky-500", value: "text-sky-500" },
    net: { icon: "text-ink-muted", value: "text-ink" },
  } as const;

  return (
    <div className="rounded-[14px] border border-surface-border/70 bg-[#FAFBFC] px-3 py-2.5">
...
function ActionButton({
  ...
  const tones = {
    income: "bg-teal-50 text-teal-brand hover:bg-teal-100/80",
    expense: "bg-rose-50 text-rose-500 hover:bg-rose-100/80",
    credit: "bg-sky-50 text-sky-600 hover:bg-sky-100/80",
  } as const;
...
export function EventCard({
  ...
  return (
    <article className="rounded-[20px] bg-white p-4 shadow-[0_4px_24px_rgba(15,23,42,0.06)]">
      <div className="flex items-start gap-3">
        ...
        <button
          ...
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-surface-border bg-white text-ink-muted shadow-sm transition-colors hover:bg-slate-50 hover:text-ink"
        >
